...
import type { MockIntervals } from '../src/discovery/mock.js';
import type { Prober } from '../src/types.js';

const fastConfig: MockIntervals = {
  probeIntervalMs: 60,
  netIntervalMs: 20,
...
test('mock fleet: seeded problems surface as attention', async () => {
  ...
  try {
    ...
    assert.ok(
      charlie.attention.some((a) => a.rule === 'unreachable' && a.severity === 'critical'),
    ...
    assert.ok(
      bravo.attention.some((a) => a.rule === 'disk'),
    ...
    assert.ok(
      delta.attention.some((a) => a.rule === 'oom'),
