...
test('probing results: 3 failures flip online to false', () => {
  ...
  const p = (): PiState => {
    const pi = f.get('p');
    if (!pi) throw new Error('pi missing');
    return pi;
  };
  f.setProbingResult('p', true);
  f.setProbingResult('p', false, 'e1');
  f.setProbingResult('p', false, 'e2');
  assert.equal(p().online, true); // only 2 failures
  f.setProbingResult('p', false, 'e3');
  assert.equal(p().online, false);
  assert.ok(p().attention.some((a) => a.rule === 'unreachable' && a.severity === 'critical'));
  f.setProbingResult('p', true);
  assert.equal(p().online, true);
  assert.equal(p().probeFailures, 0);
});
...
test('updateMetrics stores latest and fills history', () => {
  ...
  f.addPi({ id: 'p', name: 'p', ip: '1.1.1.1' });
  for (let i = 0; i < 7; i++) {
    f.updateMetrics('p', { cpuPct: i, memPct: i, diskPct: i, tempC: i, load1: i, cores: 4, uptimeSec: i });
  }
  const p = f.get('p')!;
  assert.equal(p.hist.cpu.length, 5);
  assert.equal(p.metrics!.cpuPct, 6);
...
test('updateNetwork computes bps from counter deltas', () => {
  ...
  const net = pi.net.eth0;
  assert.ok(net, 'expected eth0 sample');
  assert.ok(Math.abs(net.rxBps - 1_000_000) < 10_000); // 1 MB/s
  assert.ok(Math.abs(net.txBps - 200_000) < 10_000); // 200 KB/s
  assert.ok((pi.netHistory.eth0?.length ?? 0) >= 1);
});
